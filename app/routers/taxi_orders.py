...
async def _notify_new_taxi_order_to_brend_drivers(db: Session, order_payload: dict) -> None:
    brend_driver_ids = [
        driver_id
...
async def create_taxi_order(
    ...
    order_data_dict = {
        ...
        "dropoff_latitude": new_order.dropoff_latitude,
        "dropoff_longitude": new_order.dropoff_longitude,
    ...
    await _notify_new_taxi_order_to_brend_drivers(db, order_data_dict)
    send_new_order_push_to_working_drivers(
        db=db,
...
def delete_all_taxi_orders(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Delete all taxi orders (Admin/Superadmin only)"""
    ...
    # Get all taxi orders
    all_orders = db.query(TaxiOrder).all()
    ...
    # Delete all orders
    db.query(TaxiOrder).delete()
    db.commit()
    
    return {
        "message": f"Successfully deleted all taxi orders",
...
def bulk_delete_taxi_orders(
    delete_request: BulkDeleteRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Delete multiple taxi orders at once (only for cancelled or completed orders)"""
